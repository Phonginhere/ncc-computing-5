...
// try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        //   echo "year late: ", $_POST['skill_progress'];
        //   exit();
        if (isset($_SESSION['username'])) {
            ...
            if (
                isset($_POST['id_user']) && isset($_POST['skill_name']) && isset($_POST['skill_progress'])
            ) {
                ...
                } elseif (($_POST["skill_progress"] > 100 || $_POST["skill_progress"] < 0)) {
                    $skill_progress_error = "the skill progress is betweeen 0 and 100.";
                    // echo "check";
                    // exit();
                } else {
                ...
                if (empty(trim($_POST["skill_name"]))) {
                    $skill_name_err = "skill_name is required.";
                } else
    if (!preg_match('/^[a-zA-Z0-9, ]+$/', trim($_POST["skill_name"]))) {
                    $skill_name_err = "skill_name can only contain letters, numbers, comma, and space.";
                } elseif (strlen(trim($_POST["skill_name"])) > 60 && strlen(trim($_POST["skill_name"])) < 2) {
                    $skill_name_err = "skill_name: Maximum length is 60, minimum length is 2.";
                } else {
                    $skill_name = trim($_POST["skill_name"]);
                }

                // Check input errors before updating in database
                if (empty($skill_name_err) && empty($skill_progress_error)) {
                    // Prepare an insert statement
                    $sql = "INSERT INTO skill (skill_name, skill_progress, user_id) 
      VALUES (?, ?, ?)";

                    if ($stmt = $mysqli->prepare($sql)) {
                        // Bind variables to the prepared statement as parameters
                        $stmt->bind_param("sss", $param_skill_name, $param_progress, $param_id_user);

                        // Set parameters
                        $param_id_user = $id_user;
                        $param_progress = $skill_progress;
                        ...
                        if ($stmt->execute()) {
                            ...
                            //$success = "Your Skill info has been inserted successfully";
                            header("Location: skill.php?success=Your work experience Skill info has been inserted successfully");
                        } else {
                ...
                header("Location: insert_skill.php?error=error");
                exit;
            }
        } else {
            header("Location: login.php");
            ...
            if ($user) { ?>
                ...
                <div class="d-flex justify-content-center align-items-center vh-100">
                    ...
                    <form class="shadow w-450 p-3" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" style="width: 50%;">
                        ...
                        <h4 class="display-4  fs-1">Add Skill data</h4><br>
                        <!-- error -->
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="skill_name" placeholder="Input Skill name here....." required>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="skill_progress" id="skill_progress" required>
                            <?php if (isset($skill_progress_error)) { ?>
                                <div class="alert alert-warning alert-dismissible fade show" role="alert">
                                    <?php echo $skill_progress_error; ?>
                                    <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                                </div>
                            <?php } ?>
                        </div>
